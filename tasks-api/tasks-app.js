...
const extractAndVerifyToken = async (headers) => {
  ...
  // Use AUTH_SERVICE_URL from ConfigMap
  const authServiceUrl = process.env.AUTH_SERVICE_URL || 'http://auth-service';
  console.log(`Verifying token with: ${authServiceUrl}/verify-token/${token}`);
  
  const response = await axios.get(`${authServiceUrl}/verify-token/${token}`);
